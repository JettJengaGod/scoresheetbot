...
def calc_hardcap_current(cr: Crew) -> Tuple[int, int, int, int, float, int]:
    base_cap = 50
    players, battles = hardcap_info_current(cr, [CURRENT_LEAGUE_ID,35])
    activity = players / (cr.member_count - len(cr.crew_staff))
...
def battle_summary(bot: 'ScoreSheetBot', battle_type: BattleType) -> Optional[discord.Embed]:
    if battle_type == BattleType.MOCK:
    ...
    elif battle_type == BattleType.MIDSEASON:
        ty = 'Midseason Playoff'
    elif battle_type == BattleType.TRINITY_PLAYOFF:
        ty = 'Trinity Playoff'
        ...
        ty = 'Power'
    elif battle_type == BattleType.WISDOM_PLAYOFF:
        ty = 'Wisdom Playoff'
    elif battle_type == BattleType.COURAGE_PLAYOFF:
        ty = 'Courage Playoff'
