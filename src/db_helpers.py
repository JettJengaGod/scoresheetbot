...
from (select count(distinct (match.battle_id)) as battles
      ...
      where match.p1 = %s
        and battle.id = match.battle_id
        and battle.league_id = 35) as p1_total,
     (select count(distinct (match.battle_id)) as battle_wins
      from match,
           battle
      where match.p1 = %s
        and battle.crew_1 = battle.winner
        and battle.id = match.battle_id
        and battle.league_id = 35) as p1_wins,
     (select count(distinct (match.battle_id)) as battles
      from match, battle
      where match.p2 = %s
        and battle.id = match.battle_id
        and battle.league_id = 35) as p2_total,
     (select count(distinct (match.battle_id)) as battle_wins
      from match,
           battle
      where match.p2 = %s
        and battle.crew_2 = battle.winner
        and battle.id = match.battle_id
        and battle.league_id = 35) as p2_wins;
    """
    conn = None
    out = []
    vals = (0, 0)
